...
function incredibleSuit.onTickPowerup(p)
	...
	if data.dashTimer > 0 then
		wasDashing = true
		Defines.player_runspeed = 30

		p.keys.run = KEYS_PRESSED
		p.keys.left = false
		p.keys.right = false

		p.speedX = 30 * p.direction
		p.speedY = 0

		data.dashTimer = data.dashTimer - 1
		
		data.breakCollider.x = (p.x + p.width * 0.5) + (60 * p.direction)
		data.breakCollider.y = (p.y + p.height * 0.5) + 5
		
		-- yoinked from the rock mushroom
		for c, n in ipairs(Colliders.getColliding{a = data.breakCollider, btype = Colliders.BLOCK, filter = blockFilterSmash}) do
...
function incredibleSuit.onEnable(p, noEffects)
	p.data.incredibleSuit = {
		canDash = true,
		dashTimer = 0,
		breakCollider = Colliders.Rect(0,0,120,40,0)
	}
